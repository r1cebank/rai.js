...
function _resolve(callback) {
    ...
    if(this.promise instanceof Promise) {
        if(!this.promise.isFulfilled()) {   //  Good, the world is awesome
            AppSingleton.getInstance().L.warn("We have a unresolved promise!");
            AppSingleton.getInstance().L.warn("Waiting for promise to finish");
            var self = this;    //  Avoid this to be invalid
            this.promise.then(function (result) {
                AppSingleton.getInstance().L.info("We have a resolved promise!");
                self.output = _.assign({ }, result);
                callback();   //  Better chaining
            })
                .catch(function (e) {
                    AppSingleton.getInstance().L.error("We have a rejected promise!");
